...
function StackupOpeningScreen({ onEnter }) {
  ...
  const Suit=({type,className=""})=>{
    const p={viewBox:"0 0 64 64",className,fill:"currentColor","aria-hidden":"true"};
    if(type==="spade") return <svg {...p}><path d="M32 4C25 16 10 24 10 37c0 8 6 14 14 14 4 0 7-2 9-5-1 6-4 11-9 14h16c-5-3-8-8-9-14 2 3 5 5 9 5 8 0 14-6 14-14C54 24 39 16 32 4z"/></svg>;
    if(type==="heart") return <svg {...p}><path d="M32 57 8 34C-4 21 14 5 27 17l5 6 5-6C50 5 68 21 56 34z"/></svg>;
    if(type==="club") return <svg {...p}><path d="M32 6a12 12 0 0 0-7 22 11 11 0 1 0 3 19c-1 5-4 9-9 13h26c-5-4-8-8-9-13a11 11 0 1 0 3-19A12 12 0 0 0 32 6z"/></svg>;
    return <svg {...p}><path d="M32 4 56 32 32 60 8 32z"/></svg>;
  };
  const Icon=({type})=>{
    const p={viewBox:"0 0 64 64",fill:"none",stroke:"currentColor",strokeWidth:"2.2",strokeLinecap:"round",strokeLinejoin:"round","aria-hidden":"true"};
    if(type==="globe") return <svg {...p}><circle cx="32" cy="32" r="24"/><path d="M8 32h48M32 8c8 7 12 15 12 24S40 49 32 56M32 8c-8 7-12 15-12 24s4 17 12 24M13 20h38M13 44h38"/></svg>;
    if(type==="finger") return <svg {...p}><path d="M32 7C18 7 8 17 8 31m48 0C56 17 46 7 32 7M20 31c0-7 5-12 12-12s12 5 12 12c0 13-4 22-11 28M26 32c0-4 2-6 6-6s6 2 6 6c0 10-2 18-7 25M20 48c3-5 4-10 4-16"/></svg>;
    if(type==="wa") return <svg {...p}><path d="M12 54l4-12a22 22 0 1 1 8 8z"/><path d="M24 20c-3 2-3 6 0 11 4 7 10 11 16 12 4 1 7-1 8-4l-7-4-4 3c-5-2-8-5-11-10l3-3z"/></svg>;
    return <svg {...p} viewBox="0 0 24 24"><path d="m6 9 6 6 6-6"/></svg>;
  };

  const PokerChip=({className=""})=><div className={"pchip "+className}><i/><i/><i/><i/><span><Suit type="spade"/></span></div>;
  const Card=({id,icon,title,sub})=><button type="button" className={"entry-card "+(drawer===id?"active":"")} onClick={()=>toggle(id)}>
    <span className="entry-icon"><Icon type={icon}/></span><span className="entry-copy"><b>{title}</b><small>{sub}</small></span>
    <span className="entry-water"><Icon type={icon}/></span><span className={"entry-chevron "+(drawer===id?"up":"")}><Icon type="chev"/></span>
  </button>;
  const Drawer=({id,children})=><div className={"entry-drawer "+(drawer===id?"open":"")}><div><section>{children}</section></div></div>;

  return <div className="grinder-entry"><style>{`
@import url('https://fonts.googleapis.com/css2?family=Viga&display=swap');
*{box-sizing:border-box}.grinder-entry{min-height:100vh;background:#020202;color:#fff;overflow-x:hidden;font-family:Arial,sans-serif}.entry-shell{width:min(100%,430px);margin:0 auto;padding:0 14px 24px;background:#020202}
.premium-hero{height:430px;margin:0 -14px 10px;position:relative;overflow:hidden;background:radial-gradient(ellipse at 50% 55%,#14130f 0,#080807 35%,#020202 72%)}.premium-hero:before{content:"";position:absolute;inset:0;z-index:0;background:radial-gradient(circle at 13% 12%,rgba(232,198,117,.16) 0 2px,transparent 4px),radial-gradient(circle at 82% 9%,rgba(232,198,117,.13) 0 2px,transparent 5px),radial-gradient(circle at 68% 24%,rgba(185,137,53,.1) 0 2px,transparent 5px);filter:blur(2px)}.premium-hero:after{content:"";position:absolute;z-index:8;inset:0;pointer-events:none;background:linear-gradient(180deg,rgba(0,0,0,.05),transparent 45%,rgba(0,0,0,.18) 67%,rgba(2,2,2,.92) 98%)}
.ace-fan{position:absolute;z-index:1;left:-32px;right:-32px;top:8px;height:190px;display:flex;justify-content:center;opacity:.78;filter:brightness(.56) saturate(.68) blur(.2px)}.poker-ace{position:relative;width:101px;height:166px;flex:0 0 101px;border:1px solid #604b27;border-radius:13px;background:radial-gradient(circle at 45% 28%,#1d1b16,#080808 56%,#020202);box-shadow:inset 0 0 0 1px #29261e,inset 0 0 30px #000,0 10px 22px #000;overflow:hidden;color:#dec17a}.poker-ace:after{content:"";position:absolute;inset:0;background:linear-gradient(115deg,transparent 25%,rgba(242,217,149,.06) 44%,transparent 58%)}.ace-index{position:absolute;left:10px;top:9px;width:27px;z-index:2}.ace-index b{display:block;font:400 27px/23px Viga}.ace-index svg{display:block;width:17px;height:17px;margin:3px 0 0 2px}.poker-ace>.big-suit{position:absolute;width:56px;height:56px;left:50%;top:54%;transform:translate(-50%,-50%);opacity:.13}.poker-ace:nth-child(1){transform:rotate(-22deg) translate(28px,34px)}.poker-ace:nth-child(2){transform:rotate(-8deg) translate(6px,2px)}.poker-ace:nth-child(3){transform:rotate(8deg) translate(-6px,2px)}.poker-ace:nth-child(4){transform:rotate(22deg) translate(-28px,34px)}
.chip-stack{position:absolute;z-index:2;top:151px;width:110px;height:110px;opacity:.58;filter:brightness(.58) saturate(.58)}.chip-stack.left{left:-24px}.chip-stack.right{right:-24px;transform:scaleX(-1)}.chip-stack span{position:absolute;width:68px;height:16px;border-radius:50%;border:1px solid #70572f;background:repeating-conic-gradient(#10100f 0 13deg,#8a6329 13deg 19deg,#1b1915 19deg 34deg);box-shadow:0 10px 0 #11110f,0 20px 0 #080808,0 30px 0 #15130f,0 40px 0 #090909}.chip-stack span:nth-child(1){left:0;bottom:8px}.chip-stack span:nth-child(2){left:48px;bottom:27px}.chip-stack span:nth-child(3){left:19px;bottom:72px}
.stk-medallion{position:absolute;z-index:4;left:50%;top:24px;transform:translateX(-50%);width:222px;height:222px;border-radius:50%;background:radial-gradient(circle at 48% 40%,#17140f 0 49%,#050505 50% 64%,#18140e 65%);box-shadow:0 0 0 2px #f2d995,0 0 0 5px #0a0a09,0 0 0 9px #b98935,0 0 0 12px #050505,inset 0 0 38px #000,0 14px 34px #000}.stk-segments{position:absolute;inset:4px;border-radius:50%;background:repeating-conic-gradient(from -10deg,#090909 0 13deg,#d5aa52 13deg 23deg,#60431e 23deg 28deg,#090909 28deg 45deg);-webkit-mask:radial-gradient(circle,transparent 0 76%,#000 77%);mask:radial-gradient(circle,transparent 0 76%,#000 77%)}.stk-medallion:after{content:"";position:absolute;inset:13px;border-radius:50%;pointer-events:none;background:repeating-radial-gradient(circle at 50% 45%,rgba(242,217,149,.028) 0 1px,transparent 1px 4px);mix-blend-mode:screen}.stk-inner{position:absolute;inset:30px;border-radius:50%;border:2px solid #d5aa52;box-shadow:0 0 0 2px #050505,0 0 0 5px #60431e,inset 0 0 26px #000;background:radial-gradient(circle,#181611 0,#080807 72%)}.stk-inner>.hero-spade{position:absolute;width:132px;height:132px;left:50%;top:45%;transform:translate(-50%,-50%);color:#d5aa52;filter:drop-shadow(0 5px 2px #000)}.stk-inner>.hero-spade-outline{position:absolute;width:160px;height:160px;left:50%;top:45%;transform:translate(-50%,-50%);color:#f2d995;opacity:.28;filter:drop-shadow(0 0 1px #f2d995)}.stk-word{position:absolute;z-index:3;inset:0;display:grid;place-items:center;font:400 54px Viga;background:linear-gradient(#f2d995 0,#d5aa52 28%,#b98935 51%,#e8c675 69%,#60431e 100%);-webkit-background-clip:text;color:transparent;filter:drop-shadow(0 4px 1px #000)}
.inner-chips{position:absolute;z-index:5;left:31px;right:31px;bottom:36px;height:43px}.inner-chips i{position:absolute;width:46px;height:12px;border-radius:50%;border:1px solid #a57b36;background:repeating-conic-gradient(#0c0c0b 0 14deg,#c69b4d 14deg 21deg,#17140e 21deg 36deg);box-shadow:0 7px 0 #15130f,0 14px 0 #080808,0 21px 0 #13110d}.inner-chips i:nth-child(1){left:0}.inner-chips i:nth-child(2){left:34px;bottom:8px}.inner-chips i:nth-child(3){right:34px;bottom:8px}.inner-chips i:nth-child(4){right:0}
.pchip{position:absolute;z-index:7;right:13px;bottom:10px;width:62px;height:62px;border-radius:50%;border:7px solid #b98935;background:#080808;box-shadow:0 8px 14px #000e,inset 0 0 0 2px #f2d995;transform:rotate(-18deg)}.pchip>i{position:absolute;width:13px;height:6px;background:#e8c675;left:50%;top:2px;transform-origin:6px 31px}.pchip>i:nth-child(2){transform:rotate(90deg)}.pchip>i:nth-child(3){transform:rotate(180deg)}.pchip>i:nth-child(4){transform:rotate(270deg)}.pchip span{position:absolute;inset:16px;border:1px solid #d5aa52;border-radius:50%;display:grid;place-items:center;color:#d5aa52}.pchip span svg{width:22px;height:22px}
.hero-brand{position:absolute;z-index:9;left:0;right:0;bottom:10px;text-align:center}.hero-brand b{display:block;font-family:Viga;line-height:.88;filter:drop-shadow(0 4px 2px #000)}.hero-stackup{font-size:45px;background:linear-gradient(#fff 0,#e3e4e6 20%,#bfc1c5 40%,#707277 57%,#f5f6f7 75%,#4f5257 100%);-webkit-background-clip:text;color:transparent}.hero-holdem{font-size:32px;background:linear-gradient(#f2d995,#d5aa52 38%,#b98935 69%,#765022);-webkit-background-clip:text;color:transparent}.hero-grinder{font-size:52px;background:linear-gradient(#e1bc6a,#b98935 34%,#8a6329 59%,#5e401c 78%,#c99b45);-webkit-background-clip:text;color:transparent}.hero-brand small{display:block;margin-top:9px;color:#d7d8da;font-size:9px;letter-spacing:.25em;font-weight:700}
.entry-card:before{content:"";position:absolute;left:18px;right:18px;top:0;height:1px;background:linear-gradient(90deg,transparent,rgba(242,217,149,.55),transparent);opacity:.65}.entry-card{position:relative;width:100%;height:82px;margin:0 0 9px;padding:12px 43px 12px 16px;border:1px solid rgba(222,193,122,.75);border-radius:17px;background:linear-gradient(135deg,rgba(28,25,19,.78),rgba(5,5,5,.94));backdrop-filter:blur(16px) saturate(115%);box-shadow:inset 0 1px rgba(255,245,210,.08),0 10px 24px #000c;color:#fff;display:grid;grid-template-columns:48px 1fr;gap:13px;align-items:center;text-align:left;overflow:hidden}.entry-card.active{border-color:#f2d995}.entry-icon{width:38px;height:38px;color:#dec17a;z-index:2}.entry-icon svg,.entry-water svg,.entry-chevron svg{width:100%;height:100%}.entry-copy{z-index:2}.entry-copy b{display:block;font:400 16px Viga;color:#e8c675}.entry-copy small{display:block;margin-top:5px;color:#aaa;font-size:10px}.entry-water{position:absolute;width:118px;height:118px;right:-22px;top:-16px;color:#d5aa52;opacity:.055}.entry-chevron{position:absolute;right:14px;top:34px;width:18px;height:18px;color:#f2d995;transition:.24s}.entry-chevron.up{transform:rotate(180deg)}
.entry-drawer{display:grid;grid-template-rows:0fr;opacity:0;transform:translateY(-5px);margin:-15px 0 10px;transition:grid-template-rows .24s,opacity .22s,transform .24s}.entry-drawer.open{grid-template-rows:1fr;opacity:1;transform:none}.entry-drawer>div{overflow:hidden}.entry-drawer section{padding:19px 11px 11px;border:1px solid #79653b;border-top:0;border-radius:0 0 15px 15px;background:linear-gradient(#17140ef7,#050505fb)}.opts,.actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.choice,.action{min-height:47px;border:1px solid #665839;border-radius:10px;background:#090909;color:#eee;font:400 10.5px Viga}.choice.selected,.action.primary{border-color:#d5aa52;background:linear-gradient(135deg,#302719,#171109)}.actions{margin-top:9px}.phone-row{display:grid;grid-template-columns:84px 1fr;gap:8px}.country,.phone-input,.otp{height:47px;border:1px solid #665839;border-radius:10px;background:#070707;color:#fff}.country{display:grid;place-items:center;font:400 11px Viga}.phone-input{width:100%;padding:0 12px}.send{width:100%;margin-top:8px}.otps{display:flex;gap:8px;margin-top:10px}.otp{width:48px;height:50px;text-align:center;font-size:20px}.keep{display:flex;gap:8px;margin:11px 1px;font:400 9.5px Viga}.entry-footer{display:flex;align-items:center;gap:10px;justify-content:center;padding:10px 0 3px;color:#d5aa52;font-size:7.5px;letter-spacing:.22em;white-space:nowrap}.entry-footer:before,.entry-footer:after{content:"";height:1px;flex:1;background:linear-gradient(90deg,transparent,#8a6329)}.entry-footer:after{background:linear-gradient(90deg,#8a6329,transparent)}
@media(max-width:370px){.premium-hero{height:414px}.stk-medallion{width:236px;height:236px}.hero-stackup{font-size:45px}.hero-holdem{font-size:33px}.hero-grinder{font-size:53px}.entry-shell{padding-left:10px;padding-right:10px}.premium-hero{margin-left:-10px;margin-right:-10px}}
`}</style><main className="entry-shell">
<section className="premium-hero"><div className="ace-fan">{[["spade","A"],["heart","A"],["club","A"],["diamond","A"]].map(([t,l])=><div className="poker-ace" key={t}><div className="ace-index"><b>{l}</b><Suit type={t}/></div><Suit type={t} className="big-suit"/></div>)}</div><div className="chip-stack left"><span/><span/><span/></div><div className="chip-stack right"><span/><span/><span/></div><div className="stk-medallion"><div className="stk-segments"/><div className="stk-inner"><Suit type="spade" className="hero-spade-outline"/><Suit type="spade" className="hero-spade"/></div><div className="stk-word">STK</div><div className="inner-chips"><i/><i/><i/><i/></div><PokerChip/></div><div className="hero-brand"><b className="hero-stackup">STACKUP</b><b className="hero-holdem">HOLD'EM</b><b className="hero-grinder">GRINDER</b><small>DECIDA COM CONSISTÊNCIA</small></div></section>
<Card id="lang" icon="globe" title="IDIOMA" sub="Selecione seu idioma"/><Drawer id="lang"><div className="opts"><button className={"choice "+(draftLanguage==="pt"?"selected":"")} onClick={()=>setDraftLanguage("pt")}>PORTUGUÊS (BR)</button><button className={"choice "+(draftLanguage==="en"?"selected":"")} onClick={()=>setDraftLanguage("en")}>ENGLISH (US)</button></div><div className="actions"><button className="action" onClick={()=>{setDraftLanguage(language);setDrawer(null)}}>CANCELAR</button><button className="action primary" onClick={()=>{setLanguage(draftLanguage);setDrawer(null)}}>CONFIRMAR</button></div></Drawer>
<Card id="quick" icon="finger" title="ACESSO RÁPIDO" sub="Entre de forma rápida e segura"/><Drawer id="quick"><div className="opts"><button className="choice">BIOMETRIA</button><button className="choice">ENTRAR COM GOOGLE</button></div></Drawer>
<Card id="login" icon="wa" title="LOGIN COM WHATSAPP" sub={<>Receba um código de 4 dígitos<br/>no seu WhatsApp</>}/><Drawer id="login"><div className="phone-row"><div className="country">🇧🇷 +55 ▾</div><input className="phone-input" inputMode="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="(11) 98765-4321"/></div>{!otpSent?<button className="action primary send" onClick={()=>setOtpSent(true)}>ENVIAR CÓDIGO</button>:<><div className="otps">{otp.map((v,i)=><input key={i} className="otp" inputMode="numeric" maxLength={1} value={v} onChange={e=>{const n=[...otp];n[i]=e.target.value.replace(/\D/g,"").slice(-1);setOtp(n)}}/>)}</div><label className="keep"><input type="checkbox" checked={keepLogged} onChange={e=>setKeepLogged(e.target.checked)}/>PERMANECER LOGADO</label><div className="actions"><button className="action" onClick={()=>{setOtpSent(false);setOtp(["","","",""]);setDrawer(null)}}>CANCELAR</button><button className="action primary" onClick={()=>onEnter?.()}>CONFIRMAR</button></div></>}</Drawer>
<footer className="entry-footer">TREINE · EVOLUA · DECIDA MELHOR</footer></main></div>;
}
