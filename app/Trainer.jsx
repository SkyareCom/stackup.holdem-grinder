...
function StackupOpeningScreen({ onEnter }) {
  const [drawer,setDrawer]=useState(null), [language,setLanguage]=useState("pt"), [draftLanguage,setDraftLanguage]=useState("pt");
  const [phone,setPhone]=useState(""), [otpSent,setOtpSent]=useState(false), [otp,setOtp]=useState(["","","",""]), [keepLogged,setKeepLogged]=useState(true);
  const toggle=id=>setDrawer(v=>v===id?null:id);
  const Icon=({type})=>{const p={viewBox:"0 0 64 64",fill:"none",stroke:"currentColor",strokeWidth:"2.2",strokeLinecap:"round",strokeLinejoin:"round"};
  ...
  const Suit=({type})=>{const p={viewBox:"0 0 64 64",fill:"currentColor"}; if(type==="s")return <svg {...p}><path d="M32 5C25 16 11 23 11 36c0 8 6 13 13 13 4 0 7-2 9-5-1 7-4 11-8 15h14c-4-4-7-8-8-15 2 3 5 5 9 5 7 0 13-5 13-13C53 23 39 16 32 5z"/></svg>; if(type==="h")return <svg {...p}><path d="M32 56 9 34C-3 21 15 5 27 17l5 6 5-6C49 5 67 21 55 34z"/></svg>; if(type==="c")return <svg {...p}><path d="M32 7a12 12 0 0 0-7 22 11 11 0 1 0 3 18c-1 5-4 9-8 12h24c-4-3-7-7-8-12a11 11 0 1 0 3-18A12 12 0 0 0 32 7z"/></svg>; return <svg {...p}><path d="M32 5 55 32 32 59 9 32z"/></svg>};
  const Card=({id,icon,title,sub})=><button className={"entryCard "+(drawer===id?"active":"")} onClick={()=>toggle(id)}><span className="entryIcon"><Icon type={icon}/></span><span className="entryCopy"><b>{title}</b><small>{sub}</small></span><span className="entryWater"><Icon type={icon}/></span><span className={"entryChevron "+(drawer===id?"up":"")}><Icon/></span></button>;
  const Drawer=({id,children})=><div className={"entryDrawer "+(drawer===id?"open":"")}><div><section>{children}</section></div></div>;
  return <div className="entry"><style>{`
@import url('https://fonts.googleapis.com/css2?family=Viga&display=swap');
*{box-sizing:border-box}.entry{min-height:100vh;background:#020202;color:#fff;overflow-x:hidden;font-family:Arial,sans-serif}.entryShell{width:min(100%,430px);margin:auto;padding:0 14px 24px;background:#020202}
.hero{height:490px;margin:0 -14px 12px;position:relative;overflow:hidden;background:radial-gradient(ellipse at 50% 52%,#14130f 0,#080807 36%,#020202 72%)}.hero:after{content:"";position:absolute;inset:0;z-index:6;pointer-events:none;background:linear-gradient(180deg,rgba(0,0,0,.08),transparent 48%,rgba(0,0,0,.28) 70%,#020202 98%)}
.bokeh{position:absolute;inset:0;opacity:.18;background:radial-gradient(circle at 13% 11%,#d5aa52 0 2px,transparent 3px),radial-gradient(circle at 84% 9%,#d5aa52 0 2px,transparent 4px),radial-gradient(circle at 75% 26%,#b98935 0 1px,transparent 4px),radial-gradient(circle at 24% 29%,#b98935 0 1px,transparent 4px);filter:blur(2px)}
.aces{position:absolute;z-index:1;top:16px;left:-24px;right:-24px;height:220px;display:flex;justify-content:center;opacity:.7;filter:brightness(.53) saturate(.65) blur(.25px)}.ace{position:relative;width:112px;height:185px;border:1px solid #5b4827;border-radius:12px;background:radial-gradient(circle at 45% 35%,#181714,#070707 58%,#020202);box-shadow:inset 0 0 26px #000,0 9px 20px #000;overflow:hidden;color:#d5aa52}.aceIdx{position:absolute;left:10px;top:9px;width:24px}.aceIdx b{display:block;font:400 26px/23px Viga}.aceIdx svg{width:16px;height:16px;margin-left:2px}.ace>svg{position:absolute;width:52px;height:52px;left:50%;top:52%;transform:translate(-50%,-50%);opacity:.16}.ace:nth-child(1){transform:rotate(-21deg) translate(25px,31px)}.ace:nth-child(2){transform:rotate(-8deg)}.ace:nth-child(3){transform:rotate(8deg)}.ace:nth-child(4){transform:rotate(21deg) translate(-25px,31px)}
.sideChips{position:absolute;z-index:2;top:176px;width:110px;height:112px;opacity:.48;filter:brightness(.58) saturate(.55)}.sideChips.left{left:-25px}.sideChips.right{right:-25px;transform:scaleX(-1)}.sideChips i{position:absolute;width:65px;height:15px;border-radius:50%;border:1px solid #72572c;background:repeating-conic-gradient(#10100e 0 14deg,#8a6329 14deg 19deg,#171612 19deg 34deg);box-shadow:0 10px 0 #11110f,0 20px 0 #090909,0 30px 0 #13120f}.sideChips i:nth-child(1){left:0;bottom:8px}.sideChips i:nth-child(2){left:43px;bottom:24px}.sideChips i:nth-child(3){left:16px;bottom:57px}.sideChips i:nth-child(4){left:59px;bottom:70px}
.emblem{position:absolute;z-index:4;left:50%;top:38px;transform:translateX(-50%);width:248px;height:248px;border-radius:50%;background:radial-gradient(circle,#1b1812 0 49%,#050505 50% 65%,#16130d 66%);box-shadow:0 0 0 2px #dec17a,0 0 0 7px #080808,0 0 0 10px #8a6329,inset 0 0 32px #000,0 14px 30px #000}.seg{position:absolute;inset:5px;border-radius:50%;background:repeating-conic-gradient(from -11deg,#0b0b0a 0 14deg,#c99b45 14deg 25deg,#3a2b17 25deg 31deg,#0b0b0a 31deg 45deg);-webkit-mask:radial-gradient(circle,transparent 0 76%,#000 77% 100%);mask:radial-gradient(circle,transparent 0 76%,#000 77% 100%)}.innerRing{position:absolute;inset:32px;border-radius:50%;border:2px solid #b98935;box-shadow:0 0 0 2px #080808,0 0 0 4px #60431e,inset 0 0 22px #000}.mainSpade{position:absolute;left:50%;top:43%;transform:translate(-50%,-50%);width:142px;height:142px;color:#d5aa52;filter:drop-shadow(0 5px 2px #000)}.stkText{position:absolute;z-index:3;inset:0;display:grid;place-items:center;font:400 61px Viga;background:linear-gradient(#f2d995,#d5aa52 27%,#8a6329 58%,#e1bc6a 72%,#60431e);-webkit-background-clip:text;color:transparent;filter:drop-shadow(0 4px 1px #000)}.mini{position:absolute;z-index:4;left:38px;right:38px;bottom:43px;height:45px}.mini i{position:absolute;width:45px;height:11px;border-radius:50%;border:1px solid #9c7533;background:repeating-conic-gradient(#10100e 0 15deg,#b98935 15deg 21deg,#17140e 21deg 37deg);box-shadow:0 7px 0 #14120e,0 14px 0 #090909,0 21px 0 #12100d}.mini i:nth-child(1){left:0}.mini i:nth-child(2){left:34px;bottom:7px}.mini i:nth-child(3){right:34px;bottom:7px}.mini i:nth-child(4){right:0}.frontChip{position:absolute;z-index:5;right:22px;bottom:15px;width:68px;height:68px;border-radius:50%;border:7px dashed #d5aa52;background:radial-gradient(circle,#18140c 0 36%,#d0a44f 37% 42%,#070707 43% 61%,#b98935 62% 69%,#10100e 70%);transform:rotate(-18deg);box-shadow:0 8px 13px #000d}
.brand{position:absolute;z-index:7;left:0;right:0;bottom:13px;text-align:center}.brand b{display:block;font-family:Viga;line-height:.88;filter:drop-shadow(0 4px 2px #000)}.stackup{font-size:49px;background:linear-gradient(#fff,#e3e4e6 22%,#bfc1c5 43%,#85888e 61%,#e5e6e8 79%,#4f5257);-webkit-background-clip:text;color:transparent}.holdem{font-size:35px;background:linear-gradient(#f2d995,#d5aa52 38%,#b98935 69%,#765022);-webkit-background-clip:text;color:transparent}.grinder{font-size:58px;background:linear-gradient(#e1bc6a,#b98935 34%,#8a6329 59%,#5e401c 78%,#c99b45);-webkit-background-clip:text;color:transparent}.brand small{display:block;margin-top:10px;color:#d7d8da;font-size:9px;letter-spacing:.25em;font-weight:700}
.entryCard{position:relative;width:100%;height:86px;margin:0 0 10px;padding:12px 43px 12px 16px;border:1px solid rgba(222,193,122,.75);border-radius:17px;background:linear-gradient(135deg,rgba(28,25,19,.78),rgba(5,5,5,.94));backdrop-filter:blur(16px) saturate(115%);box-shadow:inset 0 1px rgba(255,245,210,.08),0 10px 24px #000c;color:#fff;display:grid;grid-template-columns:48px 1fr;gap:13px;align-items:center;text-align:left;overflow:hidden}.entryCard.active{border-color:#f2d995}.entryIcon{width:38px;height:38px;color:#dec17a;z-index:2}.entryIcon svg,.entryWater svg,.entryChevron svg{width:100%;height:100%}.entryCopy{z-index:2}.entryCopy b{display:block;font:400 17px Viga;color:#e8c675}.entryCopy small{display:block;margin-top:5px;color:#aaa;font-size:10px}.entryWater{position:absolute;width:118px;height:118px;right:-22px;top:-16px;color:#d5aa52;opacity:.055}.entryChevron{position:absolute;right:14px;top:34px;width:18px;height:18px;color:#f2d995;transition:.24s}.entryChevron.up{transform:rotate(180deg)}
.entryDrawer{display:grid;grid-template-rows:0fr;opacity:0;transform:translateY(-5px);margin:-15px 0 10px;transition:grid-template-rows .24s,opacity .22s,transform .24s}.entryDrawer.open{grid-template-rows:1fr;opacity:1;transform:none}.entryDrawer>div{overflow:hidden}.entryDrawer section{padding:19px 11px 11px;border:1px solid #79653b;border-top:0;border-radius:0 0 15px 15px;background:linear-gradient(#17140ef7,#050505fb)}.opts,.actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.choice,.action{min-height:47px;border:1px solid #665839;border-radius:10px;background:#090909;color:#eee;font:400 10.5px Viga}.choice.selected,.action.primary{border-color:#d5aa52;background:linear-gradient(135deg,#302719,#171109)}.actions{margin-top:9px}.phoneRow{display:grid;grid-template-columns:84px 1fr;gap:8px}.country,.phoneInput,.otp{height:47px;border:1px solid #665839;border-radius:10px;background:#070707;color:#fff}.country{display:grid;place-items:center;font:400 11px Viga}.phoneInput{width:100%;padding:0 12px}.send{width:100%;margin-top:8px}.otps{display:flex;gap:8px;margin-top:10px}.otp{width:48px;height:50px;text-align:center;font-size:20px}.keep{display:flex;gap:8px;margin:11px 1px;font:400 9.5px Viga}.footer{display:flex;align-items:center;gap:10px;justify-content:center;padding:12px 0 3px;color:#d5aa52;font-size:7.5px;letter-spacing:.22em;white-space:nowrap}.footer:before,.footer:after{content:"";height:1px;flex:1;background:linear-gradient(90deg,transparent,#8a6329)}.footer:after{background:linear-gradient(90deg,#8a6329,transparent)}
@media(max-width:370px){.hero{height:475px}.emblem{width:228px;height:228px}.stackup{font-size:44px}.holdem{font-size:33px}.grinder{font-size:52px}.entryShell{padding-left:10px;padding-right:10px}.hero{margin-left:-10px;margin-right:-10px}}
`}</style><main className="entryShell"><section className="hero"><div className="bokeh"/><div className="aces">{[["s","A"],["h","A"],["c","A"],["d","A"]].map(([t,l])=><div className="ace" key={t}><div className="aceIdx"><b>{l}</b><Suit type={t}/></div><Suit type={t}/></div>)}</div><div className="sideChips left"><i/><i/><i/><i/></div><div className="sideChips right"><i/><i/><i/><i/></div><div className="emblem"><div className="seg"/><div className="innerRing"/><span className="mainSpade"><Suit type="s"/></span><div className="stkText">STK</div><div className="mini"><i/><i/><i/><i/></div><div className="frontChip"/></div><div className="brand"><b className="stackup">STACKUP</b><b className="holdem">HOLD'EM</b><b className="grinder">GRINDER</b><small>DECIDA COM CONSISTÊNCIA</small></div></section>
...
<Card id="login" icon="wa" title="LOGIN COM WHATSAPP" sub={<>Receba um código de 4 dígitos<br/>no seu WhatsApp</>}/><Drawer id="login"><div className="phoneRow"><div className="country">🇧🇷 +55 ▾</div><input className="phoneInput" inputMode="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="(11) 98765-4321"/></div>{!otpSent?<button className="action primary send" onClick={()=>setOtpSent(true)}>ENVIAR CÓDIGO</button>:<><div className="otps">{otp.map((v,i)=><input key={i} className="otp" inputMode="numeric" maxLength={1} value={v} onChange={e=>{const n=[...otp];n[i]=e.target.value.replace(/\D/g,"").slice(-1);setOtp(n)}}/>)}</div><label className="keep"><input type="checkbox" checked={keepLogged} onChange={e=>setKeepLogged(e.target.checked)}/>PERMANECER LOGADO</label><div className="actions"><button className="action" onClick={()=>{setOtpSent(false);setOtp(["","","",""]);setDrawer(null)}}>CANCELAR</button><button className="action primary" onClick={()=>onEnter?.()}>CONFIRMAR</button></div></>}</Drawer><footer className="footer">TREINE · EVOLUA · DECIDA MELHOR</footer></main></div>;
}
